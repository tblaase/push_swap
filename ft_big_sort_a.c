...
/*   Updated: 2021/09/04 17:33:26 by tblaase          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** stack_a = NULL when finished */
{
	...
	{
		i = 0;
		while ((*stack_a)->next != NULL && (*stack_a)->content < (*stack_a)->next->content)// this while loop and the next two lines after it will push a sorted part to the other stack
		{
			(*stack_a)->swap = 0;
		...
		ft_rra(&temp_two, 0);
		while (temp_one->content > temp_two->content)// this loop is taking one sorted part and sort it into a already pushed part which is currently on top of the other stack
		{
		...
		{
			while ((*stack_b)->content > temp_one->content && (*stack_a)->swap == 0)// this loop will rotate until i can push the last number
			{
				(*stack_b)->swap = 1;
				ft_rb(stack_b, 1);
			}
